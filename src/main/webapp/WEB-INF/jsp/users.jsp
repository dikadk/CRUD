                ...
                <div class="table-content">
                    ...
                    <nav aria-label="Page navigation example">
                        <ul class="pagination justify-content-center">
                            <c:choose>
                                <c:when test="${usersCount%10==0}">
                                    <c:forEach begin="0" end="${(usersCount/rowsPerPage-1)<0 ? 0:usersCount/rowsPerPage-1}" var="x">
                                        <li class="page-item ${x+1==page ? 'active' : ''}"><a class="page-link"
                                                                                              href="/users/${x+1}"> ${x+1}</a>
                                        </li>
                                    </c:forEach>
                                </c:when>
                                <c:when test="${usersCount%10!=0}">
                                    <c:forEach begin="0" end="${usersCount/rowsPerPage}" var="x">
                                        <li class="page-item ${x+1==page ? 'active' : ''}"><a class="page-link"
                                                                                              href="/users/${x+1}"> ${x+1}</a>
                                        </li>
                                    </c:forEach>
